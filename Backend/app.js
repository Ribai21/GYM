...
app.post("/api/payment", async (req, res) => {
  ...
  try {
      ...
      await db.promise().execute("UPDATE userdetails SET is_paid = 1, payment_date = NOW() WHERE id = ?", [user_id]);
  ...
  db.promise()
    ...
    .then(([paymentRows]) => {
      if (paymentRows.length > 0) {
        ...
        //   );
        console.log("Payment record already exists for this user. Updating instead.");
    ...
    .then(() => {
      ...
      return db
        .promise()
        .execute("UPDATE userdetails SET is_paid = 1, payment_date = NOW() WHERE id = ?", [user_id]);
    })
    .then(() => {
